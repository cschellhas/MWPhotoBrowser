...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Fertig", nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
- (CGRect)frameForToolbarAtOrientation {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone &&
        self.view.frame.size.width > self.view.frame.size.height) height = 32;
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        if (_gridController.selectionMode) {
            self.title = NSLocalizedString(@"Wähle Fotos", nil);
        } else {
            NSString *photosText;
            if (numberOfPhotos == 1) {
                photosText = NSLocalizedString(@"Foto", @"Used in the context: '1 photo'");
            } else {
                photosText = NSLocalizedString(@"Fotos", @"Used in the context: '3 photos'");
            }
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            self.title = [NSString stringWithFormat:@"%lu %@ %lu", (unsigned long)(_currentPageIndex+1), NSLocalizedString(@"von", @"Used in the context: 'Showing 1 of 3 items'"), (unsigned long)numberOfPhotos];
        }
	} else {
		self.title = nil;
	}
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        // Toolbar
        _toolbar.frame = CGRectOffset([self frameForToolbarAtOrientation], 0, animatonOffset);
